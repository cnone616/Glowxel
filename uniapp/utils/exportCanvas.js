...
export async function exportCanvasAsImage(options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (cellSize < 5) {
      reject(
        new Error(
          `画布尺寸过大(${width}x${height})且颜色过多(${colorStats.size}种)，建议分块导出单个看板以获得更清晰的图纸`,
        ),
      );
      return;
    }
    ...
    if (cellSize < 7 && (width > 300 || height > 300)) {
      console.warn(
        `图纸较大且颜色较多，导出的图纸可能不够清晰。建议分块导出单个看板。`,
      );
    }
    ...
    if (canvasWidth > maxCanvasSize || canvasHeight > maxCanvasSize) {
      reject(
        new Error(
          `Canvas 尺寸过大: ${canvasWidth}x${canvasHeight}，画布: ${width}x${height}，颜色数: ${colorStats.size}`,
        ),
      );
      return;
    }
