...
export const useProjectStore = defineStore('project', {
  ...
  getters: {
    ...
    editableProjects: (state) => {
      return state.projects.filter(p => (p.status === 'draft' || p.status === 'rejected') && p.source !== 'collected')
    },

    // 自建项目（排除收藏的）
    createdProjects: (state) => {
      return state.projects.filter(p => p.source !== 'collected')
    },

    // 收藏的项目
    collectedProjects: (state) => {
      return state.projects.filter(p => p.source === 'collected')
    }
  },
  ...
  actions: {
    ...
    // 默认权限（自建项目）
    _defaultPermissions() {
      return { canEdit: true, canPublish: true, canExport: true, canDownload: true, canAssist: true }
    },

    // 收藏项目权限
    _collectedPermissions() {
      return { canEdit: false, canPublish: false, canExport: false, canDownload: false, canAssist: true }
    },
    ...
    // 添加收藏项目（只读）
    addCollectedProject({ name, width, height, palette, thumbnail, artworkId, originalAuthor }) {
      // 检查是否已收藏过同一作品
      const existing = this.projects.find(p => p.source === 'collected' && p.artworkId === artworkId)
      if (existing) {
        return { success: false, message: '已收藏过该作品', projectId: existing.id }
      }
      ...
      const newProject = {
        ...
        thumbnail: thumbnail || '',
        size: `${width}x${height}`,
        colorCount: palette ? palette.length : 0,
        status: 'published',
        rejectReason: '',
        description: '',
        icon: 'picture',
        ...
        permissions: this._collectedPermissions()
      }
      this.projects.unshift(newProject)
      this.saveToStorage()
    ...
    // 检查项目是否可编辑
    isProjectEditable(projectId) {
      const p = this.getProject(projectId)
      if (!p) return false
      if (p.source === 'collected') return false
      if (p.permissions && !p.permissions.canEdit) return false
      return p.status === 'draft' || p.status === 'rejected'
    },

    // 检查项目是否可导出
    isProjectExportable(projectId) {
      const p = this.getProject(projectId)
      if (!p) return false
      if (p.source === 'collected') return false
      if (p.permissions && !p.permissions.canExport) return false
      return true
    },

    // 检查项目是否可发布
    isProjectPublishable(projectId) {
      const p = this.getProject(projectId)
      if (!p) return false
      if (p.source === 'collected') return false
      if (p.permissions && !p.permissions.canPublish) return false
      return p.status === 'draft' || p.status === 'rejected'
    },
