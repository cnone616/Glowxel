<template>
  <view class="ble-page">
    ...
    <view class="header">
      ...
      <text class="header-title">蓝牙配网</text>
    ...
    <scroll-view scroll-y class="content">
      ...
      <view v-if="step === 1" class="card">
        ...
        <view v-if="!scanning && devices.length === 0" class="empty-state">
          ...
          <text class="empty-hint">请确保 Glowxel 处于配网模式（WiFi 未配置状态）</text>
        </view>

        <view class="action-btn" @click="startScan">
          <text class="btn-text">{{ scanning ? '搜索中...' : '开始搜索' }}</text>
        </view>
      </view>
      ...
      <view v-if="step === 2" class="card">
        <view class="card-title-section">
          ...
          <text class="card-subtitle">已连接到 {{ connectedDevice?.name }}</text>
        </view>

        <view class="form-group">
          <text class="form-label">WiFi 名称 (SSID)</text>
          <input class="form-input" v-model="wifiSSID" placeholder="请输入 WiFi 名称" />
        </view>

        <view class="form-group">
          <text class="form-label">WiFi 密码</text>
          <input class="form-input" v-model="wifiPassword" type="password" placeholder="请输入 WiFi 密码" />
        </view>
        ...
        <view class="action-btn" :class="{ disabled: sending }" @click="sendWifiConfig">
...
.header-left, .header-right { width: 60rpx; }
.header-title { font-size: 36rpx; font-weight: 700; color: var(--color-text-primary); }

.content { flex: 1; padding: 32rpx; }
...
.step.done .step-num { background-color: #4F7FFF; color: #fff; }
.step-text { font-size: 20rpx; color: var(--text-secondary); }
.step.active .step-text { color: #4F7FFF; }

.step-line {
  width: 80rpx; height: 4rpx;
  background-color: var(--border-primary);
  margin: 0 16rpx; margin-bottom: 28rpx;
}
.step-line.active { background-color: #4F7FFF; }
...
.card-title-section { margin-bottom: 32rpx; }
.card-title { font-size: 28rpx; font-weight: 500; color: var(--text-primary); display: block; }
.card-subtitle { font-size: 22rpx; color: var(--text-secondary); margin-top: 8rpx; display: block; }

.scan-status {
  display: flex; align-items: center; justify-content: center;
  gap: 16rpx; padding: 32rpx 0;
}
.scan-text { font-size: 24rpx; color: var(--text-secondary); }
...
.device-list { margin-bottom: 24rpx; }
.device-item {
...
.device-item:active { border-color: #4F7FFF; }
.device-icon { flex-shrink: 0; }
.device-info { flex: 1; display: flex; flex-direction: column; gap: 4rpx; }
.device-name { font-size: 26rpx; color: var(--text-primary); font-weight: 500; }
.device-id { font-size: 18rpx; color: var(--text-secondary); font-family: monospace; }
.device-rssi { font-size: 22rpx; color: var(--text-secondary); flex-shrink: 0; }

.empty-state { text-align: center; padding: 48rpx 0; }
.empty-text { font-size: 28rpx; color: var(--text-secondary); display: block; }
.empty-hint { font-size: 22rpx; color: var(--text-tertiary); margin-top: 12rpx; display: block; }

.form-group { margin-bottom: 24rpx; }
.form-label { font-size: 24rpx; color: var(--text-secondary); margin-bottom: 12rpx; display: block; }
.form-input {
  width: 100%; padding: 24rpx;
  height: 88rpx; line-height: 40rpx;
  background-color: var(--bg-secondary);
  border: 2rpx solid var(--border-primary);
  border-radius: 16rpx;
...
.action-btn {
  margin-top: 24rpx; padding: 28rpx;
...
.success-icon { font-size: 96rpx; color: #00c853; margin-bottom: 24rpx; }
.success-title { font-size: 36rpx; font-weight: 700; color: var(--text-primary); display: block; }
.success-desc { font-size: 26rpx; color: var(--text-secondary); margin-top: 16rpx; display: block; }
.success-hint { font-size: 22rpx; color: var(--text-tertiary); margin-top: 12rpx; display: block; }
</style>