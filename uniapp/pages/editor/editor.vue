    ...
    <view class="header">
      <view class="header-left">
        ...
        <view class="header-actions">
          <view v-if="isEditable" class="icon-btn" @click="handleSave">
            <Icon name="save" :size="36" />
          </view>
          <view v-if="isExportable" class="icon-btn" @click="handleExport">
            <Icon name="download" :size="36" />
          </view>
          <view v-if="isPublishable" class="icon-btn" @click="handlePublish">
...
export default {
  ...
  computed: {
    ...
    isEditable() {
      ...
      if (this.project.permissions && !this.project.permissions.canEdit) return false
    ...
    isPublishable() {
      if (!this.project) return false
      if (this.project.source === 'collected') return false
      if (this.project.permissions && !this.project.permissions.canPublish) return false
      const status = this.project.status || 'draft'
      return status === 'draft' || status === 'rejected'
    },
  ...
  onLoad(options) {
    ...
    // 已发布/审核中项目提示只读
    else if (this.project.status === 'published' || this.project.status === 'reviewing') {
      uni.showToast({
        title: '已发布作品仅可查看和拼豆',
        icon: 'none',
        duration: 2000
  ...
  methods: {
    ...
    handlePixelClick(x, y) {
      if (!this.isEditable) {
        const msg = this.isCollectedProject ? '收藏作品不可编辑' : '已发布作品不可编辑'
