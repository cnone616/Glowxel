    ...
    <scroll-view scroll-y class="content" @scrolltolower="loadMore">
      <view v-if="filteredFollowing.length === 0" class="empty-state">
        ...
        <text class="empty-title">{{
          searchQuery ? "未找到相关用户" : getEmptyTitle()
        }}</text>
        <text class="empty-desc">{{
          searchQuery ? "试试其他关键词" : getEmptyDesc()
        }}</text>
        <button
          v-if="!searchQuery && currentCategory === 'all'"
          class="explore-btn"
          @click="goToExplore"
        >
          ...
          <view class="user-info">
            ...
            <view v-if="user.latestWork" class="latest-work">
              ...
              <text class="work-time">{{
                formatWorkTime(user.latestWork.createdAt)
              }}</text>
            </view>
          </view>
...
export default {
  ...
  methods: {
    ...
    updateCategoryCounts() {
      ...
      this.categories[1].count = this.following.filter(
        (u) => u.isMutual,
      ).length;
      this.categories[2].count = this.following.filter(
        (u) => u.isActive,
      ).length;

      const weekAgo = Date.now() - 7 * 24 * 60 * 60 * 1000;
      this.categories[3].count = this.following.filter(
        (u) => u.followTime > weekAgo,
      ).length;
    },
    ...
    getEmptyDesc() {
      switch (this.currentCategory) {
        case "mutual":
          return "关注更多用户来建立互关关系";
        case "active":
          return "你关注的用户最近都不太活跃";
        case "recent":
          return "去发现更多有趣的用户吧";
        default:
          return "去社区发现有趣的创作者吧";
