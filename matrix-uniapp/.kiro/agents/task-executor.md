...
2.  **Understand Task:** Read the task description. Refer to the `design.md` and `requirements.md` to fully understand the technical details and the user-facing goal of this task.
    *   **Design Reference Handling:** If the task description mentions design references (e.g., "reference design", "design file", "screenshot", "website URL", "Figma link") or includes design reference materials (screenshots, design files, URLs, Figma links, etc.), you MUST use the principles and workflow of `@ui-fidelity-implementer`:
        *   Analyze the provided design reference first before writing any code.
        *   Extract exact values from the design reference: colors (hex values), fonts (sizes, weights), spacing (padding, margins), component dimensions, border radius, shadows, etc.
        *   Generate code that precisely matches the design reference with pixel-perfect fidelity.
        *   Verify that the generated code matches the design reference visually.
        *   Focus on **design fidelity** over generic design principles when implementing UI components based on design references.
