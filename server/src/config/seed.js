...
async function seed() {
  ...
  const artworks = [
    [1, 1, '超级马里奥', '经典游戏角色像素还原', 'https://picsum.photos/seed/art1/200', 32, 32, 12, 1, '中等', 86, 12, 320, 45],
    [2, 2, '皮卡丘', '宝可梦系列-皮卡丘', 'https://picsum.photos/seed/art2/200', 32, 32, 8, 1, '简单', 152, 28, 580, 78],
    [3, 3, '星之卡比', '粉色小可爱', 'https://picsum.photos/seed/art3/200', 16, 16, 6, 1, '简单', 67, 5, 210, 32],
    [4, 1, '林克', '塞尔达传说主角', 'https://picsum.photos/seed/art4/200', 32, 32, 15, 1, '困难', 203, 35, 890, 120],
    [5, 4, '向日葵', '阳光下的向日葵', 'https://picsum.photos/seed/art5/200', 32, 32, 10, 1, '中等', 45, 8, 156, 22],
    [6, 3, '吃豆人', '经典街机游戏', 'https://picsum.photos/seed/art6/200', 16, 16, 4, 1, '简单', 98, 15, 420, 55],
    [7, 2, '独角兽', '梦幻独角兽', 'https://picsum.photos/seed/art7/200', 32, 32, 12, 1, '中等', 134, 20, 510, 68],
    [8, 4, '樱花树', '春天的樱花', 'https://picsum.photos/seed/art8/200', 32, 32, 8, 1, '中等', 76, 10, 280, 38],
  ];
  for (const a of artworks) {
    await db.query(
      `INSERT INTO artworks (id, user_id, title, description, cover_url, width, height, color_count, board_count, difficulty, likes, comments_count, views, collects)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE title=VALUES(title)`, a
    );
  }
  ...
  for (const t of templates) {
    await db.query(
      `INSERT INTO templates (id, name, image_url, category, size, color_count, difficulty, usage_count, description)
       VALUES (?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE name=VALUES(name)`, t
    );
  }
  ...
  for (const c of challenges) {
    await db.query(
      `INSERT INTO challenges (id, title, banner_url, description, start_date, end_date, status, participants, submissions, prize, difficulty)
       VALUES (?,?,?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE title=VALUES(title)`, c
    );
  }
  ...
  await db.query(
    `INSERT INTO firmware_versions (version, device_type, file_url, file_size, changelog)
     VALUES ('1.0.0', 'glowxel-64', 'uploads/firmware/initial.bin', 0, '初始版本')
...
seed().catch(e => { console.error('初始化失败:', e); process.exit(1); });
