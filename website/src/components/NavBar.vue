  ...
  <nav class="navbar">
    <div class="nav-container">
      <router-link to="/" class="logo">
        ...
        <span class="logo-text">Glowxel</span>
      </router-link>

      <div class="nav-links" :class="{ active: mobileMenuOpen }">
        <router-link to="/" @click="closeMobileMenu">首页</router-link>
        <router-link to="/workspace" @click="closeMobileMenu">工作台</router-link>
        ...
        <router-link to="/challenges" @click="closeMobileMenu">挑战</router-link>
      </div>
      ...
      <button class="mobile-menu-btn" @click="toggleMobileMenu" :aria-expanded="mobileMenuOpen" aria-label="菜单">
...
.logo-text { font-size: 18px; font-weight: 700; color: var(--color-text-primary); letter-spacing: 0.01em; }
...
.nav-links a:hover { color: var(--color-text-primary); background: var(--color-bg-muted); }
.nav-links a.router-link-exact-active { color: var(--color-text-primary); background: var(--color-bg-muted); }
...
.btn-login {
  padding: 7px 14px; color: var(--color-text-secondary);
  border: 1px solid var(--color-border); border-radius: var(--radius-sm);
  font-size: 13px; font-weight: 500; text-decoration: none; transition: all 0.15s;
}
.btn-login:hover { color: var(--color-text-primary); border-color: var(--color-border-hover); }

.mobile-menu-btn {
  display: none; background: none; border: none; cursor: pointer;
  padding: 6px; color: var(--color-text-primary); border-radius: var(--radius-sm);
}
.mobile-menu-btn:hover { background: var(--color-bg-muted); }
