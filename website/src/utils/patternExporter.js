 ...
 */
export async function exportPattern(pixels, width, height, projectName = '图案', usedColors = []) {
  ...
  for (let y = 0; y <= height; y += 5) {
    const py = gridStartY + y * (cellSize + gridLineWidth) + gridLineWidth / 2 + 4
  ...
  usedColors.forEach((color, index) => {
    ...
    // 颜色方块
    ctx.fillStyle = color.hex
    ctx.fillRect(statsX, currentY - colorBoxSize + 4, colorBoxSize, colorBoxSize)
    ctx.strokeStyle = '#cccccc'
    ctx.lineWidth = 1
    ctx.strokeRect(statsX, currentY - colorBoxSize + 4, colorBoxSize, colorBoxSize)
    
    // 颜色代码和数量
    ctx.fillStyle = '#000000'
    ctx.fillText(`${color.code}: ${color.count}`, statsX + colorBoxSize + 10, currentY)
