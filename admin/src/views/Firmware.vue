<template>
  <div>
    <a-card title="固件版本管理" :bordered="false">
      <template #extra>
        <a-button type="primary" @click="showUpload = true">上传新版本</a-button>
      </template>

      <a-table :dataSource="list" :columns="columns" :loading="loading" rowKey="version" :pagination="false">
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'is_force'">
            <a-tag :color="record.is_force ? 'red' : 'default'">{{ record.is_force ? '强制' : '可选' }}</a-tag>
          </template>
    ...
    <!-- 上传弹窗 -->
    <a-modal v-model:open="showUpload" title="上传固件" @ok="handleUpload" :confirmLoading="uploading" okText="上传" cancelText="取消">
      <a-form :model="form" layout="vertical">
        ...
        <a-form-item label="设备型号">
          <a-select v-model:value="form.device_type">
            <a-select-option value="glowxel-64">glowxel-64</a-select-option>
          </a-select>
        </a-form-item>
...
const form = ref({ version: '', device_type: 'glowxel-64', changelog: '', is_force: false })
const file = ref(null)
...
async function handleUpload() {
  if (!form.value.version) { message.error('请填写版本号'); return }
  if (!file.value) { message.error('请选择固件文件'); return }
  ...
  try {
    ...
    const res = await http.post('/firmware/upload', fd, { headers: { 'Content-Type': 'multipart/form-data' } })
    if (res.code === 0) {
      message.success('上传成功')
      showUpload.value = false
      form.value = { version: '', device_type: 'glowxel-64', changelog: '', is_force: false }
      file.value = null
      loadList()
    } else {
